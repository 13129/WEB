...
def global_variable (request):
	...
	tui = Blog.objects.filter (tui__id = 1) [:3]  # 表.objects.filter(a__表的字段)外键查询 热门推荐自己设定id=1的
	cname = "最新文章"
	notice=Notice.objects.all()#查询公告
	add='威海'#临时地址
	data=weath(add)
	time=timezone.now()
	cleandes=cleander()
...
def search (request):
	...
	cname = keyword
	allblog = Blog.objects.filter (title__icontains = keyword)
...
def cate (request, cate_id):
	...
	allblog = Blog.objects.filter (category__id = cate_id)  # 获取通过URL传进来的cid，然后筛选出对应文章
...
def art (request, art_id):
	show = Blog.objects.get (id = art_id)  # 查询指定ID的文章
	hot = Blog.objects.all ().order_by ('?') [:8]  # 内容下面的您可能感兴趣的文章，随机推荐
	show.click_nums = show.click_nums + 1
	show.save ()
